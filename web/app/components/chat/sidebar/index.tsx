...
import aiChatI18n from '@/i18n/lang/chat.en'
import type { AppCategory, AppCategoryDetail } from '@/models/chat'
...
const SideBar: FC<{
  ...
  const fetchAppList = async () => {
    ...
    const categoryDetails = categories.map((category: AppCategory) => {
      return {
        key: category,
        name: categoryI18n[category] ? t(`chat.category.${category}`) : category,
      }
    })
      ...
      <div className={`mt-6 ${isMobile && 'px-2'}`}>
        <div>
          <NavItem
            ...
            label = {t('chat.sidebar.chat') as string}
            isMobile = {isMobile}
        ...
        <div className='overflow-auto '>
          {categoryDetails.length > 0 && (
            <div className='mt-10'>
              <div className={groupTitleClassName}>{t('chat.sidebar.category')}</div>
              <div className={cn('mt-3 space-y-1 overflow-y-auto overflow-x-hidden')}>
                {[allAppCategory, ...categoryDetails].map((categoryDetail, index) => (
                  <NavItem
                    ...
                    isMobile = {isMobile}
                    onClick = {() => router.push(`/chat/category/${categoryDetail.key}`)}
                  />
                ))}
              </div>
            </div>
          ...
          {myAppList.length > 0 && (
            <div className='mt-10'>
              <div className={groupTitleClassName}>{t('chat.sidebar.myApp')}</div>
                ...
                {myAppList.map((app, index) => {
                  return (
                    <NavItem
                      ...
                      label = {app.name}
                      isMobile = {isMobile}
                      onClick = {() => router.push(`/chat/app/${app.id}`)}
                    />
                  )
                })}
                {/* TODO: 创建应用按钮 */}
              ...
              <Button type='primary' className='grow flex items-center !h-7' onClick={() => setShowNewAppDialog(true)}>
                ...
                <span className='text-xs'>{t('chat.sidebar.createApp')}</span>
              </Button>
            </div>
